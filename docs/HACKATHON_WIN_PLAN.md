...
- [x] Add **second pinned corpus** (smaller than explorer): `solana-labs/solana-program-library` / `token/js` — see `case-study/EXTERNAL.md`.
- [x] **Negative / hotspot fixtures** expanded (`Keypair`, `Transaction`, `sendAndConfirmTransaction`, `onAccountChange`, `Connection` + RPC member) under `tests/*/`.
...
**Install**  
`git clone` + `npm ci` + `npm run ci` — Registry: *(fill after `npx codemod registry publish`)*.
